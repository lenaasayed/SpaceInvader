...
;****************************************************
		...
		word_color equ 0fh	
		endflag   db 0
		corsor1 db 0AH,08H   ; frist x , secound y	
		COUNTER DB 0
		RECFLAG DB 0
		LAST DW 0
; Printing Strings
;****************************************************
		...
		str8 db 'wait to other one connect $','$'
...
; For inline chatting
;**********************************************************
PLAYER1_CHATCURSOR DB 017D
PLAYER2_CHATCURSOR DB 017D
...
;power ups 
		...
		POWERUP_COUNTER   dD  03ffffh    ; COUNTER
...
;ROCKET 
		...
		ROCKET_COUNTER   dD  0Fffffh    ; COUNTER
		...
		ROCKET_TODROW  EQU 0FFFFFH     ;THE MAIN VALUE TO FILL COUNTER WHEN DISAPEARE
		ROCKET_TODELETE  EQU 0FFFFFH   ;THE MAIN VALUE TO FILL COUNTER WHEN APEARE
...
compare_esc:   
	COMPARE_KEY 01H        ; Scan code of ESC key
        JNE  MAIN_RECIVE_USER_INP
        MOV CHAR1, 0FDH         ; MSG code for ESC
        SENDMSG
        JMP ENDG
MAIN_RECIVE_USER_INP:
        ...
        COMPARE_MSG 0FDH        ; MSG Code for ESC
        JNZ compare_for_chat
        JMP ENDG
compare_for_chat:
        COMPARE_MSG 0FFH            
...
EXIT_IF_F4:
        ...
        JZ CHECK_RECIEVE_F4
        GETKEY
        COMPARE_KEY 03EH        ; f4 scan
        JNE CHECK_RECIEVE_F4
        MOV CHAR1, 0FDH         ; MSG Code of f4
        SENDMSG
        RET
CHECK_RECIEVE_F4:
        RECMSG
        ...
        COMPARE_MSG 0FDH        ;MSG code for f4
        JNZ EXIT_IF_F4
        RET
...
CHECK_DOWN_1:
        ...
        JMP SENDINP1
CHECK_RIGHT_1:
        COMPARE_KEY 04DH   ; Scan code for Right arrow
        JNE PRINT_CHAR1
        MOV CHAR1, 0FAH 
        ADDBULL
        JMP SENDINP1
...
ENDINP1:
	...
	MOV IsThereAWinner,1
        RET                     ; Ends the game if f4 is clicked
CHK_PAUSE2:
        COMPARE_MSG 0FFH        ; MSG code of f1
        JNE GO2
        PAUSE
        MOV CHAR1,0
        MOV CHAR2,0
...
GO2:    COMPARE_MSG  0FCH   ; MSG code for UP arrow
        JNE CHECK_DOWN_2
        MVSH2UP
        JMP ENDINP2
CHECK_DOWN_2:
        COMPARE_MSG  0FBH   ; MSG code for DOWN arrow
        JNE CHECK_SPACE_2
        MVSH2DOWN
        JMP ENDINP2
CHECK_SPACE_2:
        COMPARE_MSG 0FAH   ; MSG code for Right Arrow
        JNE PRINT_CHAR2
        ADDBULR
        JMP ENDINP2
PRINT_CHAR2:
...
CLRGAMEMEMORY PROC
        ...
        MOV CHAR2, 0
        ; Clear chat cursors
        MOV PLAYER1_CHATCURSOR, 017D
        MOV PLAYER2_CHATCURSOR, 017D
        ; Reset winner flag
        ...
        MOV REFflag, 1D
        MOV REFCTR, 0FFFH
        ; Reset no of bullets of each one
        MOV p1_bulls, 0
        MOV p2_bulls, 0
        ; Reset bullets array
        CMP [DI], 0FEFEH
...
PRINTCHARCHAT1 PROC
        ; The chararter to be written is stored in AL
        CMP AL, 0DH     ; ASCII of Enter key
        JZ CLEAN_CHAT_AREA1
        CMP AL, 08H
        JNZ NOT_BS1
        CMP PLAYER1_CHATCURSOR,017D
        JZ  END_PRINTCHARCHAT1
        DEC PLAYER1_CHATCURSOR
        SETCURSOR PLAYER1_CHATCURSOR,PLAYER1_msg_py
        PRTCHAR 01," ", 00H
        JMP END_PRINTCHARCHAT1
NOT_BS1:
        SETCURSOR PLAYER1_CHATCURSOR,PLAYER1_msg_py
        PRTCHAR 1,AL,0FH
        INC PLAYER1_CHATCURSOR
        CMP PLAYER1_CHATCURSOR, 39D
        JNA END_PRINTCHARCHAT1
CLEAN_CHAT_AREA1:
        MOV PLAYER1_CHATCURSOR,017D
        SETCURSOR PLAYER1_CHATCURSOR,PLAYER1_msg_py
        PRTCHAR 023D," ", 00H
...
PRINTCHARCHAT2 PROC
        ; The chararter to be written is stored in CHAR2
        COMPARE_MSG 0DH   ; ASCII of Enter key
        JZ CLEAN_CHAT_AREA2
        CMP CHAR2, 08H
        JNZ NOT_BS2
        CMP PLAYER2_CHATCURSOR,017D
        JZ  END_PRINTCHARCHAT2
        DEC PLAYER2_CHATCURSOR
        SETCURSOR PLAYER2_CHATCURSOR,PLAYER2_msg_py-1
        PRTCHAR 01," ", 00H
        JMP END_PRINTCHARCHAT2
NOT_BS2:
        SETCURSOR PLAYER2_CHATCURSOR,PLAYER2_msg_py-1
        PRTCHAR 1,CHAR2,0FH
        INC PLAYER2_CHATCURSOR
        CMP PLAYER2_CHATCURSOR, 039D
        JNA END_PRINTCHARCHAT2 
CLEAN_CHAT_AREA2:
        MOV PLAYER2_CHATCURSOR,017D
        SETCURSOR PLAYER2_CHATCURSOR,PLAYER2_msg_py-1
        PRTCHAR 023D," ", 00H
...
NAME_2 PROC 
		...
		MOVE_CURSOR  07,0AH,0
        PRINTMESSAGE  MES
		MOVE_CURSOR  corsor1,corsor1+1,0
		cursor1_on
		mov si,offset playership1
		mov di,offset playership2
		MOV AX,DI
		MOV LAST,AX
TAKE_NAME:
...
cmp_back_space:			
		 COMPARE_KEY_ASCII 08H  ; cmp backspace
		 jnz cmp_enter
		 CMP  corsor1,0AH
		 JZ ASK2
		 SENDMSG 
